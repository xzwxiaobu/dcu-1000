...
没有EEDATA，一些需要掉电保存的信息只能保存在Flash中
    ...
    |                 |
    |_________________|
    |                 |  0x9D03C000 (len = 0x3C000 + 12K = 0x3F000)
    |   Mapping Code  |
    |   252KB         |  下载区
    |   (boot 12KB)   |
    |   (main 240KB)  |
    ...
    |                 |
    |_________________|
    |                 |  0x9D07B000 (len = 0x2000)
    |   8KB flash     |
    |   reserved      |  保留区
    |                 |
    |_________________|
    |                 |  0x9D07D000 (len = 0x2000)
    |  Update         |
    |  Firmware       |  升级程序
    |  8KB            |
    |_________________|
    |                 |  0x9D07F000 (len = 0x1000)
    |  Update         |
...
#define	FLASH_SIZE_WORDS		(0x80000L)			//FLASH空间的大小
...
#define	MAIN_CODE_SIZE	 		(0x3B000L)			//主程序的FLASH空间大小
...
//kseg0_upgrade_dnld
#define	DOWN_CODE_ADDR			(0x9D03C000L)		//程序映射区的起始地址
#define	DOWN_CODE_SIZE			(FLASH_BOOT_SIZE+MAIN_FLASH_DATA_SIZE+MAIN_CODE_SIZE)		//程序映射区的大小

//kseg0_upgrade_bios
#define	UPFW_CODE_ADDR			(0x9D07D000L)		//升级程序的FLASH空间起始地址
#define	UPFW_CODE_SIZE			(0x2000L)			//升级程序的FLASH空间大小
//kseg0_upgrade_bios_d
#define UPFW_FLASH_DATA_ADDR	(0x9D07F000L)		//升级程序用于保存数据的FLASH空间起始地址
#define UPFW_FLASH_DATA_SIZE	(0x1000L)			//升级程序用于保存数据的FLASH空间大小

//config
#define	config_BFC02FF0			(0xBFC02FF0L)
#define	config_BFC02FF4			(0xBFC02FF4L)
#define	config_BFC02FF8			(0xBFC02FF8L)
#define	config_BFC02FFC			(0xBFC02FFCL)
